...
fn prompt_unnamed_active_config_name(
    store: &ConfigStore,
    ui: &mut LiveRegion,
    config: &Config,
) -> Result<PromptResult<String>> {
    let generated_name = allocate_config_name(store, ConfigKind::from_config(config))?;
    let labels = unnamed_active_config_name_labels(&generated_name);
    match prompt_select(
        ui,
        section_user_management(),
        copy(
        ...
        &labels,
        0,
        &unnamed_active_config_name_helper_lines(),
    )? {
        PromptResult::Value(0) => {
            prompt_unique_config_name(ui, section_user_management(), store, &generated_name)
        }
        PromptResult::Value(1) => Ok(PromptResult::Value(generated_name)),
...
fn prompt_unique_config_name(
    ...
    loop {
        ...
        match prompt_text(
            ...
            PromptResult::Value(value) => match validate_new_config_name(store, &value) {
    ...
    loop {
        ...
        match selected {
            UserAction::UseExisting => {
                ...
                let updated = config_with_selected_user(
                    config,
                    root_api_key,
                    account_id,
                    &user.user_id,
                ...
                *account_notice =
                    selected_user_notice(&saved_name, &updated, UserKeySaveAction::Selected);
                *config = updated;
                return Ok(false);
            }
            UserAction::RegenerateAndUse => {
                ...
                *account_notice =
                    selected_user_notice(&saved_name, &updated, UserKeySaveAction::Regenerated);
                *config = updated;
                return Ok(false);
            }
...
async fn save_selected_user_to_config(
    ...
    save_user_management_config(store, &config_name, config)?;
...
fn ensure_user_management_config_name(
    ...
    if let Some(name) = config_name.as_deref() {
        return Ok(PromptResult::Value(name.to_string()));
    }

    match prompt_unnamed_active_config_name(store, ui, config)? {
...
fn save_user_management_config(
    store: &ConfigStore,
    config_name: &str,
    config: &Config,
) -> Result<()> {
...
fn save_user_management_local_config(
    ...
    if let Some(name) = config_name {
        save_user_management_config(store, name, config)
    } else {
...
fn unnamed_active_config_name_labels(generated_name: &str) -> Vec<String> {
    match Language::current() {
        Language::En => vec![
...
fn unnamed_active_config_name_helper_lines() -> Vec<String> {
    vec![
        theme::warning(copy(
            Language::current(),
...
fn unique_config_name_helper_lines() -> Vec<String> {
    vec![
        theme::muted(copy(
            Language::current(),
            "Press Enter to use the generated name, or type a custom one.",
            "按 Enter 使用生成名称，或输入自定义名称。",
        ))
        .to_string(),
    ]
}
...
mod tests {
    use super::{
        ...
        compact_status_box_width, config_select_label, config_with_selected_user,
        ...
        logo_glass_color_for_theme, main_action_labels, next_step_copy,
        ...
        root_accounts_from_value, root_api_key_helper_lines, root_key_account_selection_labels,
        ...
        save_config, save_user_management_config, saved_summary_render_parts, select_live_lines,
        ...
        user_management_server_url_helper_lines, user_tree_labels, validate_config_name,
        validate_config_name_change, validate_draft, validate_new_config_name,
        validate_new_user_id, wizard_header_lines, wordmark_gradient_color_for_theme,
    ...
    fn user_management_save_writes_named_config_and_active_config() {
        ...
        save_user_management_config(&store, "managed", &config)
